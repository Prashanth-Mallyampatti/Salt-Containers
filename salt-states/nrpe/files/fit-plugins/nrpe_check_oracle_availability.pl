#!/usr/bin/perl
#
# Nagios-Icinga check for Oracle Databases - Availability
#
...
if (grep(/ORA-/, @SYS_COMMAND)){
  ...
  print "CRITICAL: oracle error message \"$GREP[0]\"\n";
  exit 2;
}

if (!grep(/^Oracle Database/, @SYS_COMMAND)){
    print "CRITICAL: No connection to $OPTIONS{'SID'} possible\n";
    exit 2;
}
else{
  @GREP = grep(/^Oracle Database/, @SYS_COMMAND);
